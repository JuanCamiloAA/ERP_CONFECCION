...
export default function RoleCreate({ matrix, companies = [] }: Props) {
    ...
    useEffect(() => {
        if (data.display_name && !data.name) {
            setData('name', slugify(data.display_name));
        }
    ...
    return (
        <AppLayout title="Nuevo rol">
            ...
            <form onSubmit={submit} className="space-y-6">
                <PageHeader
                    title="Nuevo rol"
                    breadcrumbs={[{ label: 'Roles', href: route('roles.index') }, { label: 'Nuevo' }]}
                    action={
                        <div className="flex gap-2">
                            <Link href={route('roles.index')}><Button variant="ghost" icon={<ArrowLeftIcon className="h-4 w-4" />}>Cancelar</Button></Link>
                            <Button type="submit" loading={processing}>Crear rol</Button>
                        </div>
                    }
                />
                ...
                <Card>
                    ...
                    <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <Input
                            ...
                            error={errors.name}
                            description="Solo letras minusculas, numeros y guiones bajos"
                            required
                        />
                        {companies.length > 0 && (
                            <Select
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                                {colorPresets.map((color) => (
                                    <button
                                        ...
                                        className={`h-8 w-8 rounded-full border-2 transition-transform ${data.color === color ? 'border-slate-900 scale-110 dark:border-white' : 'border-transparent'}`}
                                ...
                                <input
                                    ...
                                    className="h-8 w-8 cursor-pointer rounded-full border border-slate-300 dark:border-slate-600"
                                />
                            </div>
                        </div>
                    </div>
                ...
                {errors.permissions && (
                    <p className="text-sm text-rose-500">{errors.permissions}</p>
                )}
            </form>
        </AppLayout>
    );
