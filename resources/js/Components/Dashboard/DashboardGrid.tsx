...
interface DashboardGridProps {
    ...
    /**
     * Layout guardado para movil (variante ":mobile"). Va aparte del de escritorio: si se
     * derivara del de escritorio, reordenar u ocultar en el telefono no sobreviviria a recargar.
     */
    initialMobileLayout?: Layout;
    cols?: number;
    rowHeight?: number;
}
...
export function DashboardGrid({
    ...
    // El constructor de widgets vive tras el middleware "super.admin": sin esta comprobacion,
    // un admin de empresa veria el boton y recibiria un 403 al tocarlo.
    ...
    // Si el usuario aun no ha ordenado en movil, se parte del layout de escritorio como base.
    const mobileBase = initialMobileLayout.length > 0 ? initialMobileLayout : initialLayout;
    const [mobileItems, setMobileItems] = useState<MobileItem[]>(() => buildMobileItems(panels, mobileBase));

    useEffect(() => {
        setMobileItems(buildMobileItems(panels, mobileBase));
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [panels, initialMobileLayout, initialLayout]);
            ...
            return (
                <div className="-mx-4 sm:-mx-6">
                    ...
                    <div className="flex flex-col gap-3 px-4 py-4 sm:px-6">
                        ...
                        {visible.map((item, index) => (
                            ...
                            >
                                ...
                                <span className="min-w-0 flex-1">
                                    {/* line-clamp en vez de truncate: con cuatro botones de 40px al lado,
                                        cortar en una sola linea dejaba nombres ilegibles ("Producido pen..."). */}
                                    <span className="line-clamp-2 block text-sm font-semibold text-slate-900 dark:text-slate-100">
                                        {labelOf(item.key)}
                                    </span>
                        ...
                        {isSuperAdmin && route().has('super-admin.dashboard-widgets.create') ? (
                            <a
